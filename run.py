...
class Game_Board:
    ...
    def __init__(self, g_size, name, player=False):
        self.size = g_size
        self.num_ships = g_size
        self.name = name
        self.player = player
        self.ships = []
        self.guesses = []
...
def game_over():
    if scores["player"] >= size or scores["computer"] >= size:
        return True
    return False
...
def round_tally(player_hit, computer_hit):
    print("-" * 45)
    print(f"{user_board.name} guessed " + f"{uc_board.last_guess()}")
    if player_hit:
        scores["player"] += 1
